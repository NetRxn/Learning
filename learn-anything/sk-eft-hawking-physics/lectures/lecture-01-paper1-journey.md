...
**But here's where it gets interesting.** Imagine we increase the flow velocity. We engineer a narrowing of the channel, or we open a dam upstream, and the water rushes faster. The speed of sound is still 1400 m/s in absolute units, but something profound happens to a sound wave trying to fight upstream. The river becomes a barrier. A wave trying to propagate upstream must now struggle against an increasingly strong current.

Eventually, we reach a critical velocity: $v = c_s$. The flow velocity equals the sound speed. Ask yourself: what happens to a sound wave trying to escape upstream against this current?
...
Think of it this way: if you are a sound wave in a supersonic region — a region where the flow exceeds the speed of sound — you can propagate downstream at full speed. But to propagate upstream, you have to swim against a current faster than you can swim. So you cannot escape. You are trapped. The horizon is not a physical wall; it is a causal barrier, defined purely by the competition between flow velocity and wave propagation speed.

**Here is the remarkable part:** This is not a metaphor or an approximation. When you write down the Euler equations for a fluid:
...
Before I show you the math, do you see why this should be true? The wave equation is the fundamental tool for describing propagation. And whenever you have waves propagating through a medium with position-dependent properties, you can describe the propagation using a metric tensor. The geometry emerges because of how waves scatter off spatial variations, not because spacetime is actually curved.
...
**Stop and parse this.** The first equation says: density changes due to advection by the background flow and compression from velocity gradients. The second says: velocity changes due to advection and pressure gradients. These are coupled. Watch what happens next.

We use the adiabatic relation: $\delta p = c_s^2 \delta \rho$. This relates pressure perturbations to density perturbations through the sound speed — a thermodynamic property of the fluid. Combining these two equations, you can eliminate the velocity perturbation and derive a scalar wave equation for the density perturbation:

$$\partial_\mu (g^{\mu \nu} \partial_\nu \delta \rho) = 0$$

**This is the magic step.** This is a wave equation of the form $\partial_\mu (g^{\mu \nu} \partial_\nu \phi) = 0$, which is exactly the equation for a scalar field in a curved spacetime with metric $g^{\mu \nu}$. And the metric tensor is:
...
Physically, this axiom says: "The system should be reversible when there is no dissipation." It is obvious in retrospect, but it is a powerful constraint. It tells you that any term in the action that depends only on $\psi_r$ and not on $\psi_a$ is forbidden. **The dissipation must come from the coupling between the two branches.** This immediately eliminates large swaths of the term space.
...
Physically, this axiom says: "Probabilities must be non-negative." It is a fundamental requirement of quantum mechanics. Violations lead to unitarity violation, ghost states, and inconsistencies. No negotiation.
...
The calculation is intricate, but the result is clean. Axiom One immediately kills terms that depend only on $\psi_r$: $c_1 = c_2 = 0$. **Just from this one axiom, we've eliminated one-third of the terms.**
...
The final result is that only two of the original nine terms survive. **The seven are ruled out by the axioms.** Only the terms proportional to $\psi_r \psi_a$ and $\dot{\psi}_r \psi_a$ remain. The theory is highly constrained. This is formalized in SKDoubling.lean — the axioms are not just stated philosophically, but verified through formal proofs that any term not satisfying the axioms leads to a contradiction or violation of a physical requirement. You have a formal machine that checks: does this term satisfy Normalization? Does it satisfy Positivity? Does it satisfy KMS? Only the terms that pass all three checks are allowed.
...
This means there is a crossover. At low surface gravity, $\Gamma_H \ll \kappa$, and the dissipative correction is small. At high surface gravity, $\Gamma_H$ can become comparable to or exceed $\kappa$, and dissipation dominates. **THIS is the key insight:** The crossover occurs at:
...
This is where the physics becomes testable. Four experimental platforms have pioneered the study of analog black holes and their radiation. Each approaches the same physics from a different angle, with different scales and sensitivities.
...
Let's pause and consolidate our understanding by returning to physical pictures. This is where the formalism gives way to intuition, where equations become stories.
